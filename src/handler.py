...
def sanitize_string_param(param_value):
    ...
    # Allow most characters but remove only the most dangerous shell metacharacters
    # Remove: semicolons, backticks, $, pipes, &&, ||, redirects
    param_value = re.sub(r'[;`$|&<>]+', '_', param_value)
    return param_value
...
def handler(job):
    ...
    # Validate numeric parameters (only check if they're actual numbers, not ranges)
    numeric_params = ['text_encoder_lr', 'unet_lr', 'network_dim', 'network_alpha', 
                     'learning_rate', 'lr_scheduler_num_cycles', 'lr_warmup_steps',
                     'train_batch_size', 'max_train_steps', 'max_data_loader_num_workers', 'http_log_every', 'steps']
    
    for param in numeric_params:
        if param in job_input and not validate_numeric_param(job_input[param]):
            return {'error': f'Invalid numeric parameter: {param}'}
    
    # Sanitize string parameters
    sanitized_params = {}
    string_params = ['instance_name', 'class_name', 'lr_scheduler', 'optimizer_type', 
                    'http_log_endpoint', 'http_log_name', 'http_log_token']
    ...
    cmd_args = [
        ...
        "--text_encoder_lr", str(job_input['text_encoder_lr']),
        "--no_half_vae",
        "--mixed_precision", "fp16",
        "--save_precision", "fp16",
        "--full_fp16",
        "--gradient_checkpointing",
        "--unet_lr", str(job_input['unet_lr']),
        "--network_dim", str(job_input['network_dim']),
        "--network_alpha", str(job_input['network_alpha']),
        "--lr_scheduler", sanitized_params.get('lr_scheduler', job_input['lr_scheduler']),
        "--learning_rate", str(job_input['learning_rate']),
        "--lr_scheduler_num_cycles", str(job_input['lr_scheduler_num_cycles']),
        "--lr_warmup_steps", str(job_input['lr_warmup_steps']),
        "--train_batch_size", str(job_input['train_batch_size']),
        "--max_train_steps", str(job_input['max_train_steps']),
        "--output_dir", "./training/model",
        "--output_name", out_id,
        "--max_data_loader_n_workers", str(job_input['max_data_loader_num_workers']),
        "--caption_extension", "txt",
        "--save_model_as", "safetensors",
        "--network_module", "networks.lora",
        "--optimizer_type", sanitized_params.get('optimizer_type', job_input['optimizer_type']),
        "--logging_dir", "./log",
        "--http-log",
        "--http-log-endpoint", sanitized_params.get('http_log_endpoint', job_input['http_log_endpoint']),
        "--http-log-name", sanitized_params.get('http_log_name', job_input['http_log_name']),
        "--http-log-token", sanitized_params.get('http_log_token', job_input['http_log_token']),
        "--http-log-every", str(job_input['http_log_every']),
        "--cache_latents",
